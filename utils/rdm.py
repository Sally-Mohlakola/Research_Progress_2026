...
def collect_rdm(scene, bounding_radius, num_samples=1024 * 1024 * 16,
    ...
    # never populated for it. Facets whose normals tilt below the horizon
    # come off worst, since the centre ray reaches them only through the
    # crown; measured on round_diamond_gia, they end up with a 1.4x
    # dynamic range where the analytic reference varies 16.9x.
    #
...
def compute_rdm(
    ...
    pre-fix checkpoints (run_09, run_10). It is not a good default: see
    collect_rdm.
    """
